...
void CCodeParser::ParseSourceFunctions( wxString code )
{
	...
	while ( 1 )
	{
		...
		{
			wxMessageBox( _("Brackets Missing in Source File!") );
			code.insert( functionEnd +1, "//The Following Block is missing a closing bracket\n//and has been set aside by wxFormbuilder\n" );
			func->SetContents("");
		}
...
wxString CCodeParser::ParseBrackets( wxString code, int& functionStart )
{
	...
	{
		wxMessageBox( _("no brackets found") );
	}
	Str = code.Mid(functionStart, index);
