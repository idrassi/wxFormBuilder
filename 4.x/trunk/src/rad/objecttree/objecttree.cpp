...
wxPanel( parent, id )
{
	...
	m_tcObjects = new wxTreeCtrl( this, wxID_ANY, wxDefaultPosition, wxDefaultSize,
...
void ObjectTree::AddChildren( PObjectBase obj, wxTreeItemId &parent, bool is_root )
{
	if ( obj->GetObjectInfo()->GetObjectType()->IsItem())
	{
		...
		else
		{
			...
			msg = wxString::Format( _("Item without object as child of \'%s:%s\'"),
	...
	else
	{
		...
		if ( is_root )
			new_parent = m_tcObjects->AddRoot( "", -1, -1, item_data );
		else
			new_parent = m_tcObjects->AppendItem( parent, "", -1, -1, item_data );
...
void ObjectTree::UpdateItem( wxTreeItemId id, PObjectBase obj )
{
	...
	wxString text = obj_name + " : " + class_name;
...
void ObjectTree::OnObjectSelected( wxFBObjectEvent &event )
{
	...
	{
		wxLogError( _("There is no tree item associated with this object.\n\tClass: %s\n\tName: %s"),
				...
				if (obj && (obj->GetClassName() == "wxMenuBar" || obj->GetClassName() == "Frame" ) )
				{
					...
					if ( me.ShowModal() == wxID_OK )
					{
						if ( obj->GetClassName() == "wxMenuBar" )
						{
							...
							AppData()->InsertObject( me.GetMenubar( AppData()->GetObjectDatabase() ),
