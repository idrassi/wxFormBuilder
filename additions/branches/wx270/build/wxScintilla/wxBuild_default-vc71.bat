...
if %1 == NULL goto END
:: Check for build all
if %1 == ALL goto LIB_BUILD_USINGDLL
if %1 == all goto LIB_BUILD_USINGDLL

:LIB_BUILD_USINGDLL
echo Building Lib's which Use DLL's...
echo.
goto LIB_DEBUG_USINGDLL

:LIB_DEBUG_USINGDLL
echo Compiling lib debug using dll...
echo.
:: Calling the VisualC++ make
nmake -f makefile.vc  BUILD=debug CFG=dll SHARED=0 OFFICIAL_BUILD=1 USE_DLL=1

echo.
:: Check for specific mode.
if %1 == null goto END
if %1 == NULL goto END
goto LIB_RELEASE_USINGDLL

:LIB_RELEASE_USINGDLL
echo Compiling lib release using dll...
echo.
:: Calling the VisualC++ make
nmake -f makefile.vc  BUILD=release CFG=dll SHARED=0 OFFICIAL_BUILD=1 USE_DLL=1
...
if %1 == null goto END
if %1 == NULL goto END
goto LIB_DEBUG_UNICODE_USINGDLL

:LIB_DEBUG_UNICODE_USINGDLL
echo Compiling lib debug unicode using dll...
echo.
:: Calling the VisualC++ make
nmake -f makefile.vc  BUILD=debug CFG=dll UNICODE=1 SHARED=0 OFFICIAL_BUILD=1 USE_DLL=1

echo.
:: Check for specific mode.
if %1 == null goto END
if %1 == NULL goto END
goto LIB_RELEASE_UNICODE_USINGDLL

:LIB_RELEASE_UNICODE_USINGDLL
echo Compiling lib release unicode using dll...
echo.
:: Calling the VisualC++ make
nmake -f makefile.vc  BUILD=release CFG=dll UNICODE=1 SHARED=0 OFFICIAL_BUILD=1 USE_DLL=1
