...
BEGIN_EVENT_TABLE(ResizablePanel, wxPanel)
  ...
  EVT_MOUSE_CAPTURE_LOST(ResizablePanel::OnMouseCaptureLost)
END_EVENT_TABLE()

IMPLEMENT_CLASS( ResizablePanel, wxPanel )

ResizablePanel::ResizablePanel(wxWindow *parent, const wxPoint& pos, const wxSize& size )
:
wxPanel(parent, -1, pos, size, wxFULL_REPAINT_ON_RESIZE | wxTAB_TRAVERSAL )
{
  m_sizing = NONE;
  m_curX = m_curY = -1;
...
// This event must be handled on windows in case the mouse
// is lost while it is captured
void ResizablePanel::OnMouseCaptureLost(wxMouseCaptureLostEvent& e)
{
	m_sizing = NONE;
	m_curX = m_curY = -1;
}
