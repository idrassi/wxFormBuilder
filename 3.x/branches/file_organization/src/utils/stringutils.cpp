...
void XMLUtils::ConvertAndChangeDeclaration( const wxString& path, const wxString& version, const wxString& standalone, wxFontEncoding encoding, bool backup )
{
	...
	else
	{
		...
		contents.replace( declStart, declEnd - declStart + 2, (const wxChar *)wxString::Format( wxT("<\?xml version=\"%s\" encoding=\"UTF-8\" standalone=\"%s\" \?>"), (const wxChar *)version.c_str(), (const wxChar *)standalone.c_str() ).c_str() );
	}
