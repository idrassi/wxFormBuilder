...
PrefsPageGUI::PrefsPageGUI( wxWindow* parent ) : _PrefsPageGUI( parent )
{
    ...
    chkUseAUI->SetValue( conf->UseAUI );
}

void PrefsPageGUI::SetValues()
{
    wxFBSettings* conf = wxFBSettings::Get();

    conf->UseAUI = chkUseAUI->IsChecked();
}

void PrefsPageGUI::OnUpdateUI( wxUpdateUIEvent& event )
{
    wxFBSettings* conf  = wxFBSettings::Get();

    conf->IsDirty = conf->UseAUI != chkUseAUI->IsChecked();
...
_PrefsDialog( parent )
{
    pnlGUI     = new PrefsPageGUI     ( tbkPrefs );
