...
wxSize kwxLinearRegulator::DoGetBestSize() const
{
  wxSize best;
  if (m_bDirOrizFlag)
    best = wxSize(200,50);
...
void kwxLinearRegulator::OnPaint(wxPaintEvent &event)
{
	...
	if (m_clientSize.GetWidth() != w || m_clientSize.GetHeight() != h)
	{
      m_clientSize = wxSize(w,h);
      ...
      m_nScaledVal = ceil((m_nRealVal - m_nMin) * coeff);
	}
...
void kwxLinearRegulator::DrawPosition()
{
	...
	if (m_bDirOrizFlag)
	{
		...
		if ( m_nScaledVal < 0 || m_nScaledVal >= m_clientSize.GetWidth() - 1)
			draw = false ;
		else
			coeff = (m_clientSize.GetWidth() - 2) / (double)(m_nMax - m_nMin);
	}
	else
	{
		m_nScaledVal = m_clientSize.GetHeight() - m_mousePosition.y;
		if ( m_nScaledVal < 0 || m_nScaledVal >= m_clientSize.GetHeight() - 1)
			draw = false ;
		else
			coeff = (m_clientSize.GetHeight() - 2) / (double)(m_nMax - m_nMin);
	}
...
void kwxLinearRegulator::SetValue(int val)
{
	...
	//GetClientSize(&w,&h);
	h = m_clientSize.GetHeight();
	w = m_clientSize.GetWidth();
