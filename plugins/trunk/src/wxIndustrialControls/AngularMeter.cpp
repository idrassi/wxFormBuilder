...
//	m_cBackColour = *wxLIGHT_GREY;
	...
	for (int i=0; i<MAXSECTORCOLOR; i++)
	  m_aSectorColor[i] = *wxWHITE;
...
wxSize kwxAngularMeter::DoGetBestSize() const
{
...
void kwxAngularMeter::OnPaint(wxPaintEvent &event)
{
	...
	// Create a memory DC
	wxBitmap membitmap_(w,h);
	wxMemoryDC dc;
	dc.SelectObject(membitmap_);
...
{
    nSector = wxMin(MAXSECTORCOLOR-1,nSector);
