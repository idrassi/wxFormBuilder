...
wxWindow * MainFrame::CreateDesignerWindow(wxWindow *parent,DataObservable *data)
{
    wxPanel *right = new wxPanel(parent,-1);
...
wxWindow * MainFrame::CreateComponentPalette (wxWindow *parent,DataObservable *data)
{
...
wxWindow * MainFrame::CreateObjectTree(wxWindow *parent,DataObservable *data)
{
    wxPanel *tree_panel = new wxPanel(parent,-1);
...
wxWindow * MainFrame::CreateObjectInspector(wxWindow *parent,DataObservable *data)
{
    wxPanel *obj_inspPanel = new wxPanel(parent,-1);
...
: wxFrame(parent,id,wxT("wxFormBuilder v.0.1"),wxDefaultPosition,wxSize(1000,800))
{
	...
	wxWindow *objectTree      = CreateObjectTree(this,data);
	wxWindow *objectInspector = CreateObjectInspector(this,data);
	wxWindow *palette         = CreateComponentPalette(this,data);
	wxWindow *designer        = CreateDesignerWindow(this,data);

	m_mgr.AddPane(objectTree,wxLEFT,wxT("OT"));
	m_mgr.AddPane(objectInspector,wxLEFT,wxT("OI"));
	m_mgr.AddPane(designer,wxCENTRE,wxT("XX"));
	m_mgr.AddPane(palette,wxTOP,wxT("XX"));


    //wxBoxSizer *sizer = new wxBoxSizer(wxVERTICAL);
    //sizer->Add(objectTree,1,wxEXPAND,0);
    //sizer->Add(objectInspector,1,wxEXPAND,0);
    //sizer->Add(paletteEditor,1,wxEXPAND,0);
    //SetSizer(sizer);
